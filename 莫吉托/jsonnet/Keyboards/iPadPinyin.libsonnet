...
//
// 这一页任何方向都支持分体（Split）：Tab 键上划进分体，分体态下再上划一次合回来
// （同一个手势，见 Components/Split.libsonnet 开头的说明）。iPad 屏宽足够，两个方向
// 都值得分体，不像 iPhone 只有横屏。
...
// ===== 分体版面要用到的新键 =====
// 数字行分体态整行压成 0 高，与它一起消失的 backspace 靠第四行补一颗 backspaceRight；
// 第三行原本的 enter 分体态隐藏，靠第五行补一颗 enterRight——两处都是「原键隐藏、
// 别处补一颗」的同一手法，具体数值见 Components/Split.libsonnet 的 iPadWidths 注释。
