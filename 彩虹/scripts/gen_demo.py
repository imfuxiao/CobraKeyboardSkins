...
SCALE = 3            # 3 倍渲染，适配高分屏
CANDIDATES = [("彩虹", True), ("菜红", False), ("采红", False), ("彩鸿", False), ("才宏", False)]
PREEDIT_TEXT = "cai'hong"
...
BOARD_BACKGROUND = "#EDEAF3"
TITLE = "彩虹"
TITLE_COLOR = "#332E40"
AUTHOR = "作者：morse"
AUTHOR_COLOR = "#8A82A0"
SHADOW = (95, 85, 125, 70)
# 标题下面那条彩虹丝带，十档色相与皮肤色板同一套（README「一 主色板」）
RIBBON = ["#F2517A", "#F86B4F", "#F5993C", "#EDC13F", "#A6CB4C",
          "#4CBE7C", "#3EBDD1", "#4098EC", "#6E6BE2", "#A55FDC"]
RIBBON_TOP = 157
RIBBON_HEIGHT = 7
...
def compose_board(keyboard: Image.Image) -> Image.Image:
    ...
    # 彩虹丝带：十档色相等宽平铺，两端切成圆角，像表带上那截织纹
    ribbon_w = KEYBOARD_WIDTH * BOARD_SCALE
    ribbon_h = RIBBON_HEIGHT * BOARD_SCALE
    ribbon = Image.new("RGB", (ribbon_w, ribbon_h), BOARD_BACKGROUND)
    rd = ImageDraw.Draw(ribbon)
    for i, color in enumerate(RIBBON):
        rd.rectangle(
            (round(i * ribbon_w / len(RIBBON)), 0,
             round((i + 1) * ribbon_w / len(RIBBON)), ribbon_h),
            fill=color,
        )
    ribbon_mask = Image.new("L", (ribbon_w, ribbon_h), 0)
    ImageDraw.Draw(ribbon_mask).rounded_rectangle(
        (0, 0, ribbon_w - 1, ribbon_h - 1), ribbon_h // 2, fill=255
    )
    board.paste(ribbon, (left, RIBBON_TOP * BOARD_SCALE), ribbon_mask)
