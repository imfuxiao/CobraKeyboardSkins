// 「Noctua」皮肤入口 —— 只负责声明「要出哪些文件」，具体内容全在 Keyboards/ 下。
//
...
// 拼音键盘是唯一一种 iPhone 与 iPad 版面不同的键盘：
// iPhone 是 26 键四行，iPad 是全键盘五行（多一排数字、键面双标签）。
// 其余三种键盘四个场景共用同一份版面，只有高度与间距不同。
// 这里包一层，好让下面的产物矩阵对四种键盘一视同仁。
local Pinyin = {
  new(device='iPhone', isPortrait=true)::
    if device == 'iPad' then iPadPinyin.new(isPortrait=isPortrait)
    else iPhonePinyin.new(isPortrait=isPortrait),
};
...
//
// 符号页挂在 `noctuaSymbolic` 这个自定义类型下，而不是内置的 `symbolic`——
...
local builders = {
  pinyin: Pinyin,
  numeric: Numeric,
  noctuaSymbolic: Symbolic,
  numberPad: NumberPad,
};

// 文件名规则：iPhone 是「类型 + 方向」，iPad 是「iPad + 首字母大写的类型 + 方向」。
// 与仓库里其他皮肤（default / 彩虹 / Gboard）的命名一致。
