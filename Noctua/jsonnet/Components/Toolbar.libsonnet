...
//
// 本皮肤的候选条：首选做成一颗主棕药丸，其余只按「候选字 > 注释 > 序号」分三档轻重，
// 最右边一颗 › 展开成纵排。
...
local candidateCellStyle(insets) = {
  ...
  // 首选那颗棕色药丸。棕是整块键盘上最重的一档，候选条上也只给首选这一处。
...

{
  ...
  new(insets={}):: {
                     ...
                     [horizontalListName]: {
                       ...
                       // 注：引擎的 separatorColor 只对 verticalCandidates 生效（keys.md 5.1），
                       // 横排候选之间画不出分隔线，所以这一条上只靠首选的药丸分主次。
