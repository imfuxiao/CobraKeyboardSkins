...
出图分两步：先按皮肤画出键盘，再贴到带皮肤名和作者名的背景板上（见「预览板」一节）。
    ...
    cd Skins/Noctua && make compile && python3 scripts/gen_demo.py
...
# 本皮肤的底板是实打实的米色（见 README「键盘整体背景」），不像 Gboard 那样整块透明，
# 所以出图时不必另外垫一层系统键盘底：键盘图自己那块底就是真机上看到的底。
#
# 只有预览板（标题那张卡片）的底要另算：它得与键盘底板拉开一档，键盘的轮廓才看得出来。
# 取法是把底板色往白里提一半——跟着 Palette 走，换配色时不用回来改这里。
BOARD_MIX_TO_WHITE = 0.5

WIDTH = 390          # 按 iPhone 13 / 14 / 15 的屏宽出图
SCALE = 3            # 3 倍渲染，适配高分屏
CANDIDATES = [("猫头鹰", True), ("毛头", False), ("冒头", False), ("茂通", False), ("帽筒", False)]
PREEDIT_TEXT = "mao'tou'ying"
...
def label_style(skin: dict, name: str, suffix: str) -> dict:
    """按键节点 -> 它某一层前景的样式节点。

    多数键直接就有 <按键名><suffix> 这个节点。走条件前景的键（中英切换）是例外：
    它的 foregroundStyle 是一张条件表，指向 ...ChineseLabel / ...EnglishLabel，
    <按键名>Label 没人引用，已被 Style.prune 丢掉。这里改从条件表里挑一支。
    """
    style = skin.get(name + suffix)
    if isinstance(style, dict) or suffix != "Label":
        return style if isinstance(style, dict) else {}
    refs = skin.get(name, {}).get("foregroundStyle")
    if isinstance(refs, str):
        refs = [refs]
    if not isinstance(refs, list) or not refs:
        return {}
    # 条件表里挑「常态」那一支：本皮肤的条件都写成「命中时换成另一套」，
    # 所以 conditionValue 为 false 的那一支就是没命中时该画的东西。
    pick = next(
        (r for r in refs if isinstance(r, dict) and r.get("conditionValue") is False),
        refs[0],
    )
    ref = pick.get("styleName") if isinstance(pick, dict) else pick
    return skin.get(ref, {}) if isinstance(ref, str) else {}


def cell_text(skin: dict, name: str, suffix: str) -> str:
    style = label_style(skin, name, suffix)
...
def mix(color: str, other: str, t: float) -> str:
    """两个 #RRGGBB 按比例混合，t=0 全取 color，t=1 全取 other。"""
    a = [int(color[i:i + 2], 16) for i in (1, 3, 5)]
    b = [int(other[i:i + 2], 16) for i in (1, 3, 5)]
    return "#%02X%02X%02X" % tuple(round(x * (1 - t) + y * t) for x, y in zip(a, b))


def over(color: str, under: str) -> str:
    """把 #RRGGBBAA 压到不透明底上；6 位色原样返回。

    预编辑区与工具栏区的底是「底板色压到近乎全透明」（见 Theme 的渐隐），
    PIL 的 RGB 画布吃不下 alpha，这里按真机的叠法压回不透明色。
    """
...
def draw_keyboard(skin: dict) -> Image.Image:
    ...
    # 按键区的底板色。它是不透明的，over() 在这里其实只是防手滑。
    ground = over(skin["keyboardBackground"]["normalColor"], "#FFFFFF")
    ...
    for r, row in enumerate(rows):
        ...
        for name, width in zip(names, widths):
            ...
            for suffix, default_size in (("Label", 20), ("SecondaryLabel", 13), ("Badge", 10)):
                ...
                if not text:
                    continue
                node = label_style(skin, name, suffix)
...
AUTHOR = "作者：morse"
SHADOW = (60, 64, 67, 60)
# 标题下面那条线取候选首选那颗药丸的底色（也就是 Palette 的主棕），改配色时它跟着变
RIBBON_TOP = 157
RIBBON_HEIGHT = 6
...
def board_palette(skin: dict) -> dict:
    ...
    return {
        # 板底比键盘底板浅一半，键盘因此有个轮廓
        "background": mix(ground, "#FFFFFF", BOARD_MIX_TO_WHITE),
        "title": skin["qButtonLabel"]["normalColor"],
        "author": skin["preeditText"]["textColor"],
        # 首选的**底色**而不是字色：本皮肤首选是「棕底米字」，取字色会得到一条米白的线，
        # 画在米白的板子上等于没画。
        "accent": skin["horizontalCandidateStyle"]["preferredBackgroundColor"],
    }
